...
function sanitizeClobMessage(raw) {
    ...
    if (raw.includes('404') && raw.toLowerCase().includes('no orderbook exists')) {
        return null;
    }
    try {
        ...
        const errMsg = parsed.data?.error || parsed.error || parsed.statusText || 'unknown error';

        // Skip 404 Orderbook errors (common for new markets)
        if (status === 404 && errMsg.includes('No orderbook exists')) {
            return null;
        }
...
const logger = {
    ...
    interceptConsole() {
        const handle = (originalFn, logFn) => (...args) => {
            ...
            if (sanitized === null) return; // Skip ignored errors
