...
async function _doExecSafeCall(to, data, description = '') {
    if (description) logger.info(`CTF: exec safe tx — ${description}`);
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (attempt < MAX_RETRIES) {
                logger.warn(`CTF: transaction failed (attempt ${attempt}/${MAX_RETRIES}): ${friendly} — retrying in ${RETRY_DELAY / 1000}s...`);
...
async function ensureUsdcApproval(amountWei) {
    ...
    logger.success('CTF: USDC approved to CTF contract');
}
...
export async function splitPosition(conditionId, amountUsdc, negRisk = false) {
    ...
    if (shares < MIN_SHARES_PER_SIDE) {
        throw new Error(
            `Trade size too small: ${shares} shares per side (minimum is ${MIN_SHARES_PER_SIDE}). ` +
            `Increase the USDC amount so each side is at least ${MIN_SHARES_PER_SIDE} shares.`,
        );
    }

    if (config.dryRun) {
        logger.info(`CTF[SIM]: split $${amountUsdc} USDC → ${shares} YES + ${shares} NO @ $0.50 each`);
        return shares;
    }
    ...
    logger.success(`CTF: split $${amountUsdc} USDC → ${shares} YES + ${shares} NO @ $0.50`);
    return shares;
}
...
export async function mergePositions(conditionId, sharesPerSide) {
    if (config.dryRun) {
        ...
        logger.info(`CTF[SIM]: merge ${sharesPerSide} YES+NO → $${recovered} USDC recovered`);
        return recovered;
    }
    ...
    logger.success(`CTF: merged — recovered $${sharesPerSide} USDC`);
...
export async function cleanupOpenPositions(clobClient) {
    logger.info('CTF: scanning for leftover positions to clean up...');
    ...
    try {
        if (!config.dryRun) {
            ...
            if (Array.isArray(openOrders) && openOrders.length > 0) {
                logger.warn(`CTF: cancelling ${openOrders.length} dangling open order(s)...`);
                for (const order of openOrders) {
                    try { await clobClient.cancelOrder({ orderID: order.id ?? order.order_id }); } catch { /* ignore */ }
                }
                logger.success('CTF: all open orders cancelled');
            }
        }
    } catch (err) {
        logger.warn('CTF: could not fetch open orders:', err.message);
    }
    ...
    try {
        ...
        logger.warn('CTF: could not fetch positions from Data API:', err.message);
        return;
    }

    if (dataPositions.length === 0) {
        logger.info('CTF: no open positions found — starting clean');
        return;
    }

    logger.warn(`CTF: found ${dataPositions.length} open position(s) — attempting to merge back to USDC...`);
    ...
    for (const [conditionId, tokens] of byCondition) {
        try {
            ...
            if (!denominator.isZero()) {
                logger.info(`CTF: conditionId ${conditionId.slice(0, 10)}... already resolved — skipping (redeemer will handle)`);
                continue;
            }
            ...
            if (nonZero.length < 2) {
                logger.info(`CTF: conditionId ${conditionId.slice(0, 10)}... balance too low to merge — skipping`);
                continue;
            }

            // Use the minimum balance across both sides as the merge amount
            const minShares = Math.min(...nonZero.map((b) => b.shares));
            logger.warn(`CTF: merging ${minShares.toFixed(3)} YES+NO → USDC for ${conditionId.slice(0, 10)}...`);

            if (!config.dryRun) {
                await mergePositions(conditionId, minShares);
                mergedCount++;
            } else {
                logger.info(`CTF[SIM]: would merge ${minShares.toFixed(3)} shares for ${conditionId.slice(0, 10)}...`);
            }
        } catch (err) {
            logger.error(`CTF: failed to clean up ${conditionId.slice(0, 10)}... — ${parseOnchainError(err)}`);
        }
    }

    if (mergedCount > 0) {
        logger.success(`CTF: cleanup complete — merged ${mergedCount} position(s) back to USDC`);
    } else {
        logger.info('CTF: cleanup done — nothing needed merging');
    }
}
 ...
 *
 * Can be called periodically to redeem resolved CTF positions.
 */
